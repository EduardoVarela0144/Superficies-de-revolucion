...
x = [-10:0.1:10];                           %vector con valores para variable x r(y)
%función f(x,y)=0
...
h1 = plot3(x,y1,z, 'r' ,'LineWidth',2);
hold on;
h2 = plot3(x,y2,z, 'k' ,'LineWidth',2);
hold on;
...
axis equal
grid on
title('Ecuación de superficie hiperboloide de dos hojas : x^2 - y^2 - z^2 = 1 con eje de revolución en x ');
...
e = legend([h1,h2],'f(x,y) = 0, y = $$\sqrt{x.^2-1}$$','f(x,y) = 0, y = - $$\sqrt{x.^2-1}$$' )
